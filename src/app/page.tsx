    ...
    <div className="container">
      ...
      <div className="section">
        ...
        <div className="card" style={{padding:0,overflow:"hidden"}}>
          {logs?.map(l=>(
            <div className="log-item" key={l._id}>
              <span className="log-time">{new Date(l.timestamp).toLocaleString("ru")}</span>
              <span className={`log-type badge ${logBadge(l.eventType)}`}>{l.eventType}</span>
    ...
    <div className="card">
      ...
      {actions.map((action:any,i:number)=>(
        <div key={action._id} style={{fontSize:13,color:"var(--text2)",marginBottom:6,paddingLeft:i>0?16:0,borderLeft:i>0?"2px solid var(--accent)":"none"}}>
          ...
          {action.replyKeyword&&<span> · ждёт ответ: <strong>{action.replyKeyword}</strong></span>}
          <div style={{marginTop:4,background:"var(--bg)",padding:"6px 10px",borderRadius:6}}>{action.message}</div>
          {action.buttons?.length>0&&<div style={{marginTop:4}}>{action.buttons.map((b:any,bi:number)=><span key={bi} className="badge badge-yellow" style={{marginRight:4}}>🔗 {b.text}: {b.url}</span>)}</div>}
        </div>
      ))}
    </div>
  );
