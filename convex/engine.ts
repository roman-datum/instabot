...
export const handleDm = internalAction({
  ...
  handler: async (ctx, { senderId, recipientId, text }) => {
    // Find integration by recipientId (our IG account that received the DM)
    let integ = await ctx.runQuery(internal.queries.getIntegrationByInstagramId, { instagramId: recipientId });
    if (!integ) {
      // Fallback: try all integrations (sender/recipient might be app-scoped differently)
      const all = await ctx.runQuery(internal.queries.getAllIntegrations);
      integ = all.find((i: any) => i.instagramId !== senderId) || all[0];
    }
    ...
    await ctx.runMutation(internal.mutations.addLog, { clientInstagramId: senderId, eventType: "dm_received", message: `[@${integ.pageName}] ${text}` });
...
export const handleComment = internalAction({
  ...
  handler: async (ctx, { commentId, text, senderId, mediaId }) => {
    // Try all integrations — the comment webhook doesn't directly tell us which account
    ...
    for (const integ of integrations) {
      ...
      await ctx.runMutation(internal.mutations.ensureClient, { instagramId: senderId, token: integ.pageAccessToken });
      await ctx.runMutation(internal.mutations.addLog, { clientInstagramId: senderId, eventType: "comment_received", message: `[@${integ.pageName} post:${mediaId}] ${text}` });
      ...
      break; // Found matching integration, stop
    }
  },
});
