...
export const exchangeAndSave = internalAction({
  ...
  handler: async (ctx, { code }) => {
    ...
    // Step 3: Get CORRECT user IDs from /me endpoint
    const profileRes = await fetch(
      `https://graph.instagram.com/v19.0/me?fields=user_id,username,name,id&access_token=${longToken}`
    );
    const profile = await profileRes.json();
    console.log("Profile:", JSON.stringify(profile));

    // profile.id = app-scoped IG user ID (used for API calls)
    // profile.user_id = Instagram Business Account ID  
    const igAppScopedId = profile.id || profile.user_id;

    // Step 4: Save integration with CORRECT ID
    await ctx.runMutation(internal.mutations.saveIntegration, {
      accessToken: longToken,
      pageAccessToken: longToken,
      pageId: igAppScopedId,
      instagramId: igAppScopedId,
